...
//thiago messias
//fatec luigi papaiz
//desafio de tecnicas de programação II
//juntar o padrao flyWeight com um script de pizzaria

let flyWeightPedidos = [''];
...
function montarPizza() {
    ...
    flyWeightPedidos.push(descricao += "   - " + sabor + "<br>   - " + tamanho + "<br>   - " + borda + extra + opcionais)
...
class Flyweight {
    ...
    operation(extrinsicState) {
        if (flyWeightPedidos[0] != null) {

            const pedidohtml = `
            <p>${this.intrinsicState} - ${extrinsicState} </p>
            `

            document.querySelector("#pedidosHTML").insertAdjacentHTML("beforeend", pedidohtml)
...
class Client {
    ...
    run() {
        ...
        const PedidoA = this.flyWeightFactory.getFlyweight('Pedido A')
        PedidoA.operation(flyWeightPedidos[1])

        const PedidoB = this.flyWeightFactory.getFlyweight('Pedido B')
        PedidoB.operation(flyWeightPedidos[2])
